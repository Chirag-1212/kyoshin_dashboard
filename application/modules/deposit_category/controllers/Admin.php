...
class Admin extends Auth_controller
{
	...
	public function __construct()
	{
		...
		$this->redirect = 'deposit_category/admin/';
		$this->title = 'Deposit Category';
	...
	public function all($page = '')
	{
		...
		$data = array_merge($this->data,[
			...
			'form_link' => $this->redirect . 'add/',
			'form_check_value' => 'add',
			'delete_link' => $this->redirect . 'soft_delete/',
			'delete_check_value' => 'soft_delete',
			'edit_link' => $this->redirect . 'edit/',
			'edit_check_value' => 'edit',
...
// 		$data['offset'] = $page;
// 		$data['title'] = 'Deposit Category';
// 		$data['page'] = 'list';
// 		$data['type'] = '';
// 		$data['deposit'] = 'deposit-category';
// 		$data = array_merge($this->data, $data);
			...
			if ($this->form_validation->run()) {
			    if (strlen($_FILES['featured_image']['name']) > 0) {
					...
					if (isset($data['detail']->CoverImage) && $data['detail']->CoverImage != '') {
						$featured_image = $data['detail']->CoverImage;
					} else {
						$featured_image = "";
					}
				}
	...
	{   
		...
		$data['title'] = 'Add Deposit Category';
		$data['page'] = 'form';
		$data['doc_path'] = $doc_path;
		$data['deposit'] = 'deposit-category';
		$data['html'] = $this->get_parents_html($selected_parent = '');
	...
	public function get_childs($childs = array(), $selected_parent, $space)
	{
		...
		if (!empty($childs)) {
			foreach ($childs as $key => $value) {
				...
				$new_childs = $this->db->order_by('rank', 'ASC')->get_where($this->table, array('parent_id' => $value->id, 'status' => '1'))->result();
				if (!empty($new_childs)) {
					$space = $space . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	...
	public function menuTree($parent_id = 0, $html = '')
	{
		...
		$menus = $this->db->order_by('rank', 'ASC')->get_where($this->table, array('parent_id' => $parent_id, 'status' => '1', 'show_on_menu' => 'Yes', 'show_type !=' => 'BOTTOM'))->result();
		...
		if (count($menus) > 0) {
			...
			foreach ($menus as $row) {
				...
				$subMenus = $this->db->order_by('rank', 'ASC')->get_where($this->table, array('parent_id' => $row->id, 'status' => '1', 'show_on_menu' => 'Yes', 'show_type !=' => 'BOTTOM'))->result();
				if (count($subMenus) > 0) {
	...
	public function menu()
	{
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				$slug = $this->crud_model->createUrlSlug($this->input->post('PageTitle'));
				$check_slug = $this->crud_model->get_where_single($this->table, array('PageName' => $slug));
				if (empty($check_slug)) {
					$data['PageName'] = strtolower($slug);
				} else {
					$data['PageName'] = strtolower($slug) . time();
				}
				$data['created_by'] = $this->userId;
