        ...
        <div class="col-md-12">
            <div class="box">
                ...
                <div class="box-body">
                        ...
                        <tbody>
                            ...
                            if ($items) {
                                ...
                                foreach ($items as $key => $value) {
                                    $offset = $offset + $i;
                                    if ($value->status == '1') {
                                        $status = '<span class="label label-success">CSR only</span>';
                                    }else if ($value->status == '3') {
                                        $status = '<span class="label label-success">Popup & CSR </span>';
                    ...
                    // Compare duration date with current date
                    $current_date = date('Y-m-d'); // Get current date
                    if (strtotime($value->due_date) >= strtotime($current_date)): ?>
                        <span class="label label-danger">New</span>
                    <?php endif; ?></td>
                               <td> <?php echo $value->show_pop; ?>  <?php //$csr_type=$this->crud_model->get_where_single('csr_type', array('id' => $value->csr_type_id,'status'=>'1'));   echo isset($csr_type->Title)?$csr_type->Title:''; ?></td>
