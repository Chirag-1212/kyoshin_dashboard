...
// 		}
	...
	public function get_parents_html($selected_parent = '')
	{
		...
		$parents = $this->db->order_by('rank', 'ASC')->get_where('deposit_category', array('status' => '1', 'parent_id' => 0))->result();
		if ($parents) {
			foreach ($parents as $key => $value) {
				$html  .= '<option value="' . $value->id . '" ' . (((isset($value->id)) && $value->id == $selected_parent) ? "selected" : "") . '>' . $value->PageTitle . '</option>';
				$childs = $this->db->get_where('deposit_category', array('parent_id' => $value->id, 'status' => '1'))->result();
				if (!empty($childs)) {
					$space = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	...
	public function get_childs($childs = array(), $selected_parent, $space)
	{
		...
		if (!empty($childs)) {
			foreach ($childs as $key => $value) {
				...
				$new_childs = $this->db->order_by('rank', 'ASC')->get_where('deposit_category', array('parent_id' => $value->id, 'status' => '1'))->result();
				if (!empty($new_childs)) {
					$space = $space . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
