	...
	{ 
		...
		if($this->input->post()){
			...
			if($this->form_validation->run()){
				...
				// 	}
				    $check_unique_reference_number = $this->crud_model->get_where_single($this->table,array('reference_number'=>$this->input->post('reference_number')));
				    ...
				    $check_unique_reference_number = $this->crud_model->get_where_single($this->table,array('reference_number'=>$this->input->post('reference_number'),'id !=' => $id));
				    if(empty($check_unique_reference_number)){
						$data['reference_number'] = $this->input->post('reference_number');
					}else{
						$this->session->set_flashdata('error', 'Reference Number Should be unique');
				...
				//     	$data['created_by'] = $this->current_user->id; 
						...
						if ($file_name != '') {
							...
							for ($i = 1; $i < count($fp1); $i++) {
								...
								$branch_id = $this->db->get_where('tbl_branch',array('branch_code'=>$issued_branch_code))->row()->id;
								$currency_id = $this->db->get_where('currency',array('code'=>$currency_code))->row()->id;
                                ...
                                if($branch_id && $currency_id){
                                    ...
                                    $check_unique_reference_number = $this->crud_model->get_where_single($this->table,array('reference_number'=>$reference_number)); 
