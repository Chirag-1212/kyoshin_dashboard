...
class Admin extends Auth_controller {
	...
	public function search($page = '')
	{
		...
		$all_data = $this->crud_model->count_all_data($this->table, $data_filter);
				...
				// var_dump($all_data);
				// exit;
		$config['base_url'] = base_url($this->redirect . 'search');
		...
		$branches = $this->crud_model->get_where_order_by('branches', array('status' => '1'), 'PageTitle', 'DESC');
		$reference_numbers = $this->crud_model->get_sql_all_no_pagination('bank_guarantee',  array('status' => '1'), 'reference_number', 'ASC', 'reference_number');
...
// 	}
    ...
    public function all($page = '')
	{
		...
		$data['reference_numbers']  =$this->crud_model->get_sql_all_no_pagination('bank_guarantee',  array('status' => '1'), 'reference_number', 'ASC', 'reference_number');
		$data['branches'] = $this->crud_model->get_where_order_by('branches', array('status' => '1'), 'PageTitle', 'ASC'); 
		$data['title'] = $this->title;
		$data['page'] = 'list';
		$data['redirect'] = $this->redirect;
		$data['bank_guarantee'] = 'bank_guarantee-all';
				...
				//     	$data['created_by'] = $this->current_user->id; 
						...
						if ($file_name != '') {
							...
							for ($i = 1; $i < count($fp1); $i++) {
								...
								$branch_detail = $this->db->get_where('branches',array('branch_code'=>$issued_branch_code))->row();
...
// 	Updates from Excell
	...
	public function updates_from_excel($id = '')
	{ 
		...
		if ($this->input->post()) {   
				...
				if (strlen($_FILES['FileName']['name']) > 0) {
					...
					if (!$this->upload->do_upload('FileName')) {
						...
						$this->session->set_flashdata('error', $this->upload->display_errors()); 
					    redirect($this->redirect . 'updates_from_excel'); 
					} else {
        ...
        $data['title'] = 'Add ' . $this->title;
		$data['doc_path'] = 'uploads/bank_guarantee/';
		$data['page'] = 'updates_from_excel';
		$this->load->view('layouts/admin/index', $data);
	}
	
	public function edit_from_excel($id = '')
	{ 
						...
						if ($file_name != '') {
							...
							for ($i = 1; $i < count($fp1); $i++) {
                                ...
                                if($branch_id && $currency_id){
                                    ...
                                    $check_reference_number = $this->crud_model->get_where_single($this->table,array('reference_number'=>$reference_number, 'status' => '1')); 
                				    ...
                				    if($check_reference_number){ 
                                        ...
                                        $final_data[] = $data1;
            							if($this->crud_model->update($this->table, $data1,array('reference_number'=>$reference_number))){
            							    $updatedrow++;
            							}
        									$successmsg= 'Successfully Updated.Total Updated Rows : '. $updatedrow ;
                			
                					}else{
                					    $errormsg .= 'Cannot find reference number. Reference number : '.$reference_number.'<br>';
                				// 	    $this->session->set_flashdata('error', 'Cannot find reference number. Reference number : '.$reference_number);
        						      //  redirect($this->redirect . 'edit_from_excel'); 
                					} 
                                }else{
                                    $this->session->set_flashdata('error', 'Invalid currency Code or Branch Id');
        						    redirect($this->redirect . 'edit_from_excel'); 
